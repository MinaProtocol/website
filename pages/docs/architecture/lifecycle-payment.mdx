...
In the Bitcoin network, a transaction is confirmed with 99.9% confidence after [6 blocks]( https://en.bitcoin.it/wiki/Confirmation) (60 mins) assuming that an attacker is unlikely to amass more than 10% of the hashrate.
...
The recommended wait time for a transaction to be confirmed is 15 blocks which provides a 99.9% confidence (similar to Bitcoin) that the transaction will not be reversed.
