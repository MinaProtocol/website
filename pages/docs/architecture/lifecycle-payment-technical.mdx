...
If you scroll to the bottom of [client.ml](https://github.com/minaprotocol/mina/tree/master/src/app/cli/src/client.ml), you'll find we register the `send-payment` command to the function `send-payment`.
...
In the body of `send-payment` we build a payment and send it over to the [daemon](#daemon).
...
At the bottom of [mina_main.ml](https://github.com/minaprotocol/mina/tree/master/src/app/cli/src/mina_main.ml), we define a `Run` functor that finally has the other side of the `rpc` call that the client makes to `send-payment`. Run contains the server-side implementations of all the RPC calls the client makes. It also is responsible for logic of setting up any RPC/webservers servers and background processes.
...
Back in [mina_main.ml](https://github.com/minaprotocol/mina/tree/master/src/app/cli/src/mina_main.ml), we invoke `send-payment` in [Run](#run), that delegates to `schedule_payment` -- here we enqueue the payment into the [Transaction Pool](#transaction-pool).
